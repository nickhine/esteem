...
class ActiveLearningDriver:
    """
    Active Learning Driver class
    """
    ...
    def __init__(self) -> None:
        ...
        self.all_mltrain_tasks = {}
        self.all_mltraj_tasks = {}
    ...
    def get_traj_from_calc(self,calc):
        calc = calc[-1]
        if calc not in self.calc_to_traj_map:
            raise Exception(f'Unknown calculator label {calc}. Expected calculator labels: {self.calc_to_traj_map}')
        return self.calc_to_traj_map[calc]

    def get_ssm_from_traj(self,traj):
        if traj not in self.traj_to_ssm_map:
            raise Exception(f'Unknown trajectory label {traj}. Expected trajectory labels: {self.traj_to_ssm_map}')
        return self.traj_to_ssm_map[traj]

    def get_gen_from_calc(self,calc):
        try:
            return int(calc[6:-1])
        except ValueError:
            return None
        ...
        for basis in basis_sets:
            for func in funcs:
                for target in targets:
                    ...
                    self.solutes_task.target = target
        ...
        for t in train_calcs:
            for tp in [t]:
                for target in targets:
                    ...
                    task.exc_dir_suffix = f'{targets[target]}_{meth}{self.pref(t)}_{traj_suffix}'
                    task.output = f'{truth}_{self.suff(tp)}'
                    task.carved_suffix = f'carved_{self.suff(tp)}'
    ...
    def add_trajectories(self,task,seeds,calc,traj_suffixes,dir_suffixes,ntraj,targets,target,truth):
        """
    ...
    def add_iterating_trajectories(self,task,seeds,calc,iter_dir_suffixes,targets,target,meth,truth,only_gen=None):
        """
        Adds iterating trajectories
        """
        gen = self.get_gen_from_calc(calc)
        genstart = 0
        genend = gen
        if isinstance(task,MLTestingTask) and gen is not None:
                ...
                for dir_suffix in iter_dir_suffixes:
                    ...
                    for seed in seeds:
                        ...
                        for ikey,key in enumerate(all_keys):
                            ...
                            traj_type_char = self.get_traj_from_calc(calc)
                            if (ikey==1):
                                traj_type_char = 'Q'
                            ...
                            traj_link_dir = f"{seed}_{targstrp}_{meth}{self.pref(calcp)}_{dir_suffix}"
    ...
    def create_mltrain_tasks(self,train_task:MLTrainingTask,train_calcs,seeds,targets,rand_seed,meth,truth,
        ...
        for target in targets:
            for t in train_calcs:
                ...
                train_task.calc_dir_suffix = f"{meth}{self.pref(t)}"
                ...
                self.add_trajectories(train_task,seeds,t,traj_suffixes,dir_suffixes,ntraj,targets,target,truth)
                # For generations > 0, we now add chosen subset trajectories for active learning
                self.add_iterating_trajectories(train_task,seeds,t,iter_dir_suffixes,targets,target,meth,truth)
                # extra epochs for each generation
                if 'max_num_epochs' in train_task.wrapper.train_args:  # MACE specific
                    gen = self.get_gen_from_calc(t)
                    if gen is not None:
    ...
    def create_mltraj_tasks(self,mltraj_task:MLTrajTask,train_calcs,targets,rand_seed,meth,md_wrapper,
        ...
        for target in targets:
            for t in train_calcs:
                ...
                mltraj_task.calc_dir_suffix = f'{meth}{self.pref(t)}'
    ...
    def create_mltest_tasks(self,test_task:MLTestingTask,train_calcs,seeds,targets,rand_seed,truth,meth,
        ...
        for target in targets:
            for t in train_calcs:
                ...
                test_task.calc_dir_suffix = f'{meth}{self.pref(t)}'
                test_task.which_trajs = list('A')
                test_task.traj_prefix = f"{test_task.calc_seed}_{targets[target]}_{meth}{self.pref(t)}_test/"
                ...
                self.add_trajectories(test_task,seeds,t,traj_suffixes,dir_suffixes,ntraj,targets,target,truth)
                # For generations > 0, we now add chosen subset trajectories for active learning
                self.add_iterating_trajectories(test_task,seeds,t,iter_dir_suffixes,targets,target,meth,truth)
                for rs in rand_seed:
                ...
                for tp in train_calcs:
                    ...
                    if separate_valid:
                        ...
                        test_task.traj_links_valid = None
                        test_task.which_trajs_valid = None
                    only_gen = self.get_gen_from_calc(tp)
                    self.add_iterating_trajectories(test_task,seeds,t,iter_dir_suffixes,targets,target,meth,truth,only_gen=only_gen)
                    for rs in rand_seed:
                        test_task.wrapper.train_args['seed'] = rand_seed[rs]
                        test_task.calc_suffix = f'{meth}{t}{rs}'
    ...
    def create_spectra_tasks(self,spectra_task:SpectraTask,train_calcs,targets,rand_seed,meth,ntraj,traj_suffix='specdyn',corr_traj=False):
        """
        ...
        for target in targets:
            ...
            for t in train_calcs:
                ...
                spectra_task.exc_suffix = f'{targstr}_{meth}{self.pref(t)}_mldyn'
    ...
    def setup_scripts(self,scriptname,seed,targstr,num_calcs,calc_suffix,method,script_settings,make_sbatch,allseed=None):
    ...
    def unit_test(self):
        ...
        meth="MACE"
        truth="orca"
        new_clusters_tasks = self.create_clusters_tasks(clusters_task,train_calcs,targets,rand_seed,meth,truth)
        ...
        new_training_tasks = self.create_mltrain_tasks(training_task,train_calcs,targets,rand_seed,meth,truth,traj_suffixes,dir_suffixes,ntraj)
        ...
        new_mltraj_tasks = self.create_mltraj_tasks(mltraj_task,train_calcs,targets,rand_seed,meth,md_wrapper,snap_wrapper)
        pp.pprint('new_mltraj_tasks:')
        pp.pprint(new_mltraj_tasks)
        
        mltest_task = SimpleNamespace()
        mltest_task.wrapper = wrapper
        new_mltest_tasks = self.create_mltest_tasks(mltest_task,train_calcs,targets,rand_seed,truth,meth)
        ...
        new_spectra_tasks = self.create_spectra_tasks(mltest_task,train_calcs,targets,rand_seed)
