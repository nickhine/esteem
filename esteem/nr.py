...
all_mltraj_tasks.update(create_mltraj_tasks(mltraj_task,train_calcs,targets,rand_seed,meth,md_wrapper,snap_wrapper=snap_wrapper,traj_suffix='spec',two_targets=True))
mltraj_task.carve_trajectory_max_atoms = None
dist = [0.0,2.5,5.0,7.5,10.0]
for i in dist:
    for solv in all_solvents:
        solv_rad[cluster_radii['B']][solv] = i
    mltraj_task.carve_trajectory_radius = solv_rad[cluster_radii['B']]
    all_mltraj_tasks.update(create_mltraj_tasks(mltraj_task,train_calcs,targets,rand_seed,meth,md_wrapper,snap_wrapper=snap_wrapper,traj_suffix=f'specR{i}',two_targets=True))
